...
function AuthInput({ value, id, onChange, type, name, ...inputProps }) {
  ...
  return (
    ...
    >
      <label
        className={`absolute z-10 text-[15px] top-1/2 -translate-y-1/2 text-gray-600 dark:text-gray-300 transition-transform duration-300 translate-x-3 " ${(isFocus || value) && " md:text-sm"}`}
        htmlFor={id}
      >
        {name}
      </label>
